...
(eval-and-compile
  (defun minuet-ai-load-path ()
    (mapcar (lambda (x)
              (expand-file-name x (car (borg-load-path "minuet-ai"))))
            '("minuet.el" "assets"))))

(use-package minuet
  :load-path (lambda () (minuet-ai-load-path))
  ...
  (setq minuet-openai-compatible-options
        `(:name "llama.cpp"
                :end-point ,(concat "http://" (ar-emacs-gptel-llamacpp-endpoint) "/v1/completions")
                :api-key "TERM"
                :model "Mellum-4B"
